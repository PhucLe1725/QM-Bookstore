      ...
      if (error.response?.data?.code) {
        ...
        switch (errorCode) {
          ...
          case 1001: // USER_NOT_FOUND
            ...
            break
          default:
            errorMessage = error.response.data.message || 'Đã xảy ra lỗi. Vui lòng thử lại.'
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          {errors.submit && (
            <div className={`rounded-md p-4 ${errors.submit.includes('hết hạn') ? 'bg-yellow-50 border border-yellow-200' : 'bg-red-50'
              }`}>
              <div className="flex">
                <div className="flex-shrink-0">
                ...
                <div className="ml-3">
                  <p className={`text-sm ${errors.submit.includes('hết hạn') ? 'text-yellow-800' : 'text-red-800'
                    }`}>{errors.submit}</p>
                </div>
              </div>
            </div>
